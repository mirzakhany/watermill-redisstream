...
var (
	errXInfoConsumersParseFail = errors.New("xinfo consumers parse fail")
)

func parseXInfoConsumer(cc []interface{}) (res []*xInfoConsumer, err error) {
	res = make([]*xInfoConsumer, 0, len(cc))
	var (
		ci   []interface{}
		ok   bool
		key  string
	...
	for _, c := range cc {
		if ci, ok = c.([]interface{}); !ok {
			return nil, errXInfoConsumersParseFail
		} else if len(ci)%2 != 0 {
			return nil, errXInfoConsumersParseFail
		} else {
			xic := &xInfoConsumer{
			...
			for i := 0; i < len(ci); i += 2 {
				...
				case "idle":
					...
					xic.Idle = time.Duration(i64) * time.Millisecond
				default:
				}
			}
			if xic.Name == "" || xic.Pending < 1 || xic.Idle < 1 {
				return nil, errXInfoConsumersParseFail
			}
		}
	}
